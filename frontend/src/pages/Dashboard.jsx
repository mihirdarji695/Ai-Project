...
const Dashboard = () => {
  ...
  const handleFileUpload = async (event) => {
    ...
    setProcessingStatus({ status: 'processing', message: 'Processing file...' });
    ...
    try {
      ...
      setRecentActivities(prev => {
        const updated = prev.map(activity => 
          activity.id === newActivity.id 
            ? { ...activity, status: 'completed', details: `Processed ${data.topics.length} topics` }
            : activity
        );
        localStorage.setItem('recentActivities', JSON.stringify(updated));
        return updated;
      ...
      setProcessingStatus({ status: 'completed', message: 'File processed successfully' });
      ...
      setRecentActivities(prev => {
        const updated = prev.map(activity => 
          activity.id === newActivity.id 
            ? { ...activity, status: 'failed', details: 'Failed to process file' }
            : activity
        );
        localStorage.setItem('recentActivities', JSON.stringify(updated));
        return updated;
      ...
      setProcessingStatus({ status: 'failed', message: 'Failed to process file' });
  ...
  const generateAllMaterials = async () => {
    ...
    try {
      ...
      const newActivity = {
        ...
        status: 'processing',
        timestamp: new Date().toISOString(),
        details: 'Generating Question Bank, Lesson Plan, CO-PO Mapping, and Schedule'
      };
      ...
      setRecentActivities(prev => {
        const updated = prev.map(activity => 
          activity.id === newActivity.id 
            ? { ...activity, status: 'failed', details: 'Failed to generate materials' }
            : activity
        );
        localStorage.setItem('recentActivities', JSON.stringify(updated));
        return updated;
      ...
      toast.error('Failed to generate materials: ' + (error.message || 'Unknown error'));
    } finally {
    ...
    <div className="min-h-screen p-6 bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <motion.div 
          ...
          className="bg-white p-6 rounded-2xl shadow-lg border border-purple-100"
        >
          <h3 className="text-lg font-semibold text-gray-700 mb-2">Pending Files</h3>
          <p className="text-3xl font-bold text-yellow-600">{stats.pendingFiles}</p>
        </motion.div>
      </div>
        ...
        {fileData && (
          <div className="mt-4">
            ...
            <div className="text-sm text-gray-500 text-center mt-2">
              This will create all educational materials from your syllabus: Question Bank, Lesson Plan, CO-PO Mapping, and Day-wise Schedule
            </div>
            
            {localStorage.getItem('materialsGenerated') === 'true' && (
      ...
      <motion.div 
        ...
        className="bg-white p-6 rounded-2xl shadow-lg border border-purple-100"
      >
        <h3 className="text-xl font-semibold mb-4 text-indigo-800">Recent Activities</h3>
